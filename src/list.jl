...
See also: [`ConcurrentSkipList`](@docs)
"""
...
"""
    ConcurrentSkipList{T,M} <: AbstractSkipList{T,M}

A non-concurrent skip list. `T` is the type of the element that can be stored in the
`SkipList`, while `M` is the list's mode:

- `M == :List`: the same key can be stored in a skip list multiple times.
- `M == :Set`: only one copy of each key is allowed in the skip list.
  `ConcurrentSkipListSet{T}` is an alias for `ConcurrentSkipList{T,:Set}`.
...
julia> 1:10_000 |> shuffle .|> ii -> @spawn insert!(list, ii) .|> wait;
...
A concurrency-safe skip list that only allows one copy of each key. `ConurrentSkipListSet{T}` is an
alias for `ConcurrentSkipList{T,:Set}`.
"""
